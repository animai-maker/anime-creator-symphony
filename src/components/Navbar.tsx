  ...
  return (
    <nav className="w-full flex items-center justify-between py-4 px-8 animai-glass z-10">
      ...
      <div className="hidden md:flex items-center gap-8 relative">
        <div className="absolute inset-0 h-full rounded-full bg-white/10 backdrop-blur-lg -z-10 tubelight" 
             style={{
        ...
        {navItems.map((item) => (
          <Link 
            key={item.id}
            to={item.path}
            className={`relative px-4 py-2 text-animai-navy font-medium transition-colors z-10 ${
              activeItem === item.id ? 'text-animai-purple' : 'hover:text-animai-purple'
            }`}
